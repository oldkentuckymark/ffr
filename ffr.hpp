...
public:
    ...
    virtual ffr::math::vec4 operator()(const ffr::math::vec4& in) = 0;
};
...
public:
    ...
    virtual void line(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color)
    {
    ...
    virtual void lineHorizontal(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t color)
    {
    ...
    void DrawArray(DrawType drawtype, const uint32_t first, const uint32_t count)
    {
        ...
        if(vertex_size == 2)
        {
            ffr::math::vec2* vp = reinterpret_cast<ffr::math::vec2*>(vertex_pointer);
            auto ggg = vp[0].y;
        ...
        {
            ffr::math::vec3* vp = reinterpret_cast<ffr::math::vec3*>(vertex_pointer);
            for(uint32_t i = first ;i < count; ++i)
            {
        ...
        {
            ffr::math::vec4* vp = reinterpret_cast<ffr::math::vec4*>(vertex_pointer);
            for(uint32_t i = first ;i < count; ++i)
            {
...
protected:
    ...
    uint16_t xres, yres;    //160x128 = gba mode 5
    void* vertex_pointer;
    ...
    std::vector<uint8_t> ybuffer;//xres
                             ...
                             Vertex& q1new, Vertex& q2new)
    {
        ...
        if((currentInside) && (!previousInside))
        {
            ...
            q2new.pos = ffr::math::mix(previousVertex.pos, currentVertex.pos, lerpAmount);
        ...
        else if((!currentInside) && (previousInside))
        {
            math::fixed32 lerpAmount = (currentVertex.pos.w - currentComponent) /
                               ((currentVertex.pos.w - currentComponent) -
                                (previousVertex.pos.w - previousComponent));
            q1new.pos = ffr::math::mix(currentVertex.pos, previousVertex.pos, lerpAmount);
            return;
        }
        return;
    }

void terrain(math::vec2 p, math::fixed32 phi, int height, int horizon, int scale_height, int distance, int screen_width, int screen_height)
{
...
//Render( Point(0, 0), 0, 50, 120, 120, 300, 800, 600 )
    ...
    while (z < distance)
    {
        ...
        //# segment the line
        auto dx = (pright.x - pleft.x) / screen_width;
        auto dy = (pright.y - pleft.y) / screen_width;

        //# Raster line and draw a vertical line for each segment
        for (math::fixed32 i=0.0_fx; i < screen_width;i = i + 1.0_fx)
        {
            auto height_on_screen = (height - heightmap[pleft.x, pleft.y]) / z * scale_height. + horizon
...
// Assume plot(x, y) is defined elsewhere
void plot(int x, int y);

// Interpolates x at a given y using integer math
int interpolateX(const Vec2& a, const Vec2& b, int y)
{
    int dy = b.y - a.y;
    int dx = b.x - a.x;
    int deltaY = y - a.y;
    ...
    return a.x + dx * deltaY / dy;
}

// Draws a horizontal line between two x-coordinates at a given y
void drawHorizontalLine(int xStart, int xEnd, int y)
{
    if (xStart > xEnd)
    {
        int temp = xStart;
...
void drawFilledTriangle(Vec2 v0, Vec2 v1, Vec2 v2)
{
    ...
    // Fill bottom half of triangle (v0 to v1)
    for (int y = v0.y; y <= v1.y; ++y)
    {
        int xLeft  = interpolateX(v0, v2, y);
        int xRight = interpolateX(v0, v1, y);
        drawHorizontalLine(xLeft, xRight, y);
    }

    // Fill top half of triangle (v1 to v2)
    for (int y = v1.y + 1; y <= v2.y; ++y)
    {
        int xLeft  = interpolateX(v0, v2, y);
        int xRight = interpolateX(v1, v2, y);
        drawHorizontalLine(xLeft, xRight, y);
    }
}
